...
const loadCart = async (req, res) => {
  try {
    ...
    if (cartPoducts.length > 0) {
      subtotal = Math.round(
        cartPoducts[0].products.reduce((acc, val) => {
          ...
          if (product.offerprice > 0 && category.offerprice > 0) {
            return (
              acc +
              (product.offerprice -
                (product.offerprice * category.offerprice) / 100) *
                val.quantity
            );
...
const totalPrice = async (req, res) => {
  try {
    ...
    if (updateCart) {
      const fullProduct = await Cart.findOne({
        user: req.session.user_id,
      }).populate({
        path: "products.product",
        populate: {
      ...
      const subtotal = Math.round(
        fullProduct.products.reduce((acc, val) => {
          const product = val.product;
          const category = product.category;
          if (product.offerprice > 0 && category.offerprice > 0) {
            return (
      ...
      if (productPrice.offerprice > 0) {
        if (productPrice.category && productPrice.category.offerprice > 0) {
          price = Math.round(
            productPrice.offerprice -
              (productPrice.offerprice * productPrice.category.offerprice) / 100
          );
        } else {
        ...
        productPrice.category.offerprice > 0
      ) {
        const discountedPrice =
          productPrice.price -
          (productPrice.price * productPrice.category.offerprice) / 100;
      ...
      res
        .status(200)
        .json({ message: "succes", quantity, price, subtotal, quantity });
    }
  } catch (error) {
    console.log(error);
  }
...
const loadCheckout = async (req, res) => {
  try {
    ...
    const addressescount = addressData.map(
      (addressDoc) => addressDoc.addresses.length
    );
