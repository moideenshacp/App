        ...
        order.products.forEach((product) => {
          if (
            ...
            if (product.product.offerprice > 0) {
              ...
              totalDiscount +=
                order.totalAmount - product.quantity * product.product.price;
            }
          }
        });
      });
...
const addcategory = async (req, res) => {
  try {
    ...
    const existingCategory = await categories.findOne({
      name: { $regex: "^" + lowercase + "$", $options: "i" },
    });
    ...
    if (
      ...
      (isNaN(parsedOfferPrice) ||
        parsedOfferPrice < 1 ||
        parsedOfferPrice > 100)
    ) {
      const categorylist = await categories.find({});
      return res.render("category", {
        categorylist,
        messages:
          "Invalid discount provided. It must be a percentage between 1 and 100.",
      });
    }
...
const editcategory = async (req, res) => {
  try {
    ...
    if (
      parsedDiscount !== 0 &&
      (isNaN(parsedDiscount) || parsedDiscount < 1 || parsedDiscount > 100)
    ) {
      return res.render("editcategory", {
        categoryData,
        messages:
...
const productAdd = async (req, res) => {
  try {
    const { name, description, price, quantity, category, offerPrice } =
      req.body;
    ...
    if (!req.files || req.files.length !== 4) {
      ...
      return res.render("addproduct", {
        categorylist,
        message: "Select Exactly Four Images",
        name,
        description,
        price,
        quantity,
    ...
    if (offerPrice && (isNaN(offerPrice) || offerPrice <= 0)) {
      return res.render("addproduct", {
        categorylist,
        message: "Offer Price is not valid",
        name,
        description,
        price,
        quantity,
...
const editProductLoad = async (req, res) => {
  try {
    ...
    const productData = await products
      .findById({ _id: id })
      .populate("category");
    const productImage = await products.findById(
      { _id: id },
      { image: 1, _id: 0 }
    );
...
const editProduct = async (req, res) => {
  try {
    ...
    if (
      (offerPrice && (isNaN(offerPrice) || offerPrice <= 0)) ||
      offerPrice > price
    ) {
      const productData = await products.findById({ _id: id });
      const categorylist = await categories.find({});
      return res.render("editproduct", {
        ...
        productData,
        message:
          "Offer Price is not valid or offerPrice is greater than originl price",
      });
    }
    if (isNaN(quantity) || quantity < 0) {
      const productData = await products.findById({ _id: id });
    ...
    if (images && images.length > 0) {
      images.forEach((item, index) => {
        if (index <= 4) {
          // Assuming you have up to 4 image slots
...
const statusChange = async (req, res) => {
  try {
    ...
    // Find the exact product in the order
    ...
    if (action === "cancelled") {
      ...
      if (["Razorpay", "Wallet"].includes(order.paymentMethod)) {
        ...
        const refundAmount = productData.price * orderProduct.quantity;
...
const returnOrder = async (req, res) => {
  try {
    ...
    // Find the exact order with the given orderId
    const order = await Order.findOne({ _id: orderId, user: userId });
    ...
    // Find the specific product inside the order
    const orderProduct = order.products.find(
      (product) => product.product.toString() === productId
    );
    ...
    const productFind = await Product.findById(productId).populate("category");
    if (!productFind) {
    ...
    if (action === "accept") {
      ...
      productFind.quantity += orderProduct.quantity;
      productFind.sales -= orderProduct.quantity;
      await productFind.save();

      // Handle refund if paid through Razorpay or Wallet
      if (
        ...
        if (productFind.offerprice > 0 && productFind.category.offerprice > 0) {
          refundAmount =
            (productFind.offerprice -
              (productFind.offerprice * productFind.category.offerprice) /
                100) *
          ...
          const discountedPrice =
            productFind.price -
            (productFind.price * productFind.category.offerprice) / 100;
