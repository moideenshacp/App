...
const couponAdd = async (req, res) => {
  try {
    ...
    const { name, code, minamount, discount, expireDate, description } =
      req.body;
    ...
    if (!code || !/^\d{3,}$/.test(code)) {
      return res.render("couponAdd", {
        messages: "Invalid coupon code provided. It must be at least 3 digits.",
        name,
        code,
        minamount,
        discount,
    ...
    if (!minamount || isNaN(minamount) || minamount <= 0) {
      return res.render("couponAdd", {
        messages:
          "Invalid minimum amount provided. It must be a positive number.",
        name,
        code,
        minamount,
        discount,
    ...
    if (!expireDate || isNaN(Date.parse(expireDate))) {
      return res.render("couponAdd", {
        messages: "Invalid expiry date provided.",
        name,
        code,
        minamount,
        discount,
    ...
    if (new Date(expireDate) < today) {
      return res.render("couponAdd", {
        messages: "The expiry date has already passed.",
        name,
        code,
        minamount,
        discount,
    ...
    if (!description || /^\s*$/.test(description)) {
      return res.render("couponAdd", {
        messages: "Invalid discription provided.",
        name,
        code,
        minamount,
        discount,
    ...
    if (existingCouponName) {
      return res.render("couponAdd", {
        messages: "Coupon with the same name already exist",
        name,
        code,
        minamount,
        discount,
    ...
    if (existingCouponCode) {
      return res.render("couponAdd", {
        messages: "Coupon with the same code already exist",
        name,
        code,
        minamount,
        discount,
...
const editCouponLoad = async (req, res) => {
  try {
    ...
    const formattedExpireDate = couponFind.expireDate
      .toISOString()
      .split("T")[0];
...
const updateCoupon = async (req, res) => {
  try {
    ...
    if (existingCoupon) {
      return res.render("couponAdd", {
        messages: "Coupon with the same name and code already exists.",
        name,
        code,
        minamount,
        discount,
...
const applyCoupon = async (req, res) => {
  try {
    ...
    const userHasUsedCoupon = couponCheck.usedUsers.some(
    ...
    res
      .status(200)
      .json({ message: "Coupon applied successfully", discountedSubtotal });
  } catch (error) {
    console.log(error);
  }
};
...
const removeCoupon = async (req, res) => {
  try {
    ...
    const userIndex = couponCheck.usedUsers.findIndex(
      (user) => user._id.toString() === userId.toString()
    );
    if (userIndex !== -1) {
